...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        if playerCarController.view.isPaused != true {
    ...
    func constraintMove(_ view: SKSpriteNode, parent: SKNode) {
    ...
    func increasePlayerHealth(_ notification: Notification) {
        if let info = (notification as NSNotification).userInfo as? Dictionary<String,CGFloat> {
    ...
    func addPothHole() {
        if gameStage >= 3 {
            ...
            let pothHoleSpawn = SKAction.run {
            ...
            self.run(SKAction.repeatForever(pothHoleSpawnPeriod))
    ...
    func addGiftAmor() {
        if gameStage >= 2{
            ...
            let giftAmorSpawn = SKAction.run {
            ...
            self.run(SKAction.repeatForever(giftAmorSpawnPeriod))
    ...
    func addGiftBullet() {
        if gameStage >= 2{
            ...
            let giftBulletSpawn = SKAction.run {
            ...
            self.run(SKAction.repeatForever(giftBulletSpawnPeriod))
    ...
    func addGiftBomb() {
        ...
        let giftBombSpawn = SKAction.run {
        ...
        self.run(SKAction.repeatForever(giftBombSpawnPeriod))
    }
    
    
    func addGiftPower() {
        
        let giftPowerSpawn = SKAction.run {
        ...
        self.run(SKAction.repeatForever(giftPowerSpawnPeriod))
    }
    
    func addEnemy(_ speed: Double) {
    ...
    func addPlayer() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(increasePlayerHealth(_:)), name: NSNotification.Name(rawValue: "Increase Health"), object: nil)
    ...
    func addScore() {
        ...
        scoreText.fontColor = UIColor.blue
    ...
    func gameOver() {
        ...
        gameOverText.fontColor = UIColor.red
        ...
        playerCarController.view.isPaused = true
    ...
    func gameEnd() {
        ...
        self.view?.presentScene(gameEndScene, transition: SKTransition.doorsCloseHorizontal(withDuration: 1))
